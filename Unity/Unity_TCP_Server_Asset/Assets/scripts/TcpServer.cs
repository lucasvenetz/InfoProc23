...
public class TcpServer : MonoBehaviour
{
    ...
    private int[] reflection = new int[2];
    public GameObject[] PlayerX; 
    public Health[] health;
    public bool isStart;
    public bool isRestart;
    public bool[] pOut = new bool[2];
    public bool[] willClientEnd = new bool[2];
    public int[,] Keys = new int[2,7];

    void Start()
    {
        isRestart = false;
        willClientEnd = new bool[2]{false, false};
        Application.targetFrameRate = 30; 
        StartServer();
    }
    public string GetPlayerID(int player0){
        foreach (var client0 in clients.Keys){
            if (player0 == clientNumbers[client0]){
                return clients[client0];
            }
        }
    ...
    void ReadParsedData1(string key, string value, int clientNo)
    {
        ...
        else if (key == "BUT1")
        {
            ...
            Debug.Log(">>> END Player "+clientNo);
            willClientEnd[clientNo] = true;
        }
    }
    ...
    {
        if (isRestart){
            endGameButton.SubmitGameScores();
            EndOneGame();
        }
            ...
            if (isStart){
                ...
                lock (clients)
                {
                    foreach (var pair in clients)
                    {
                        ...
                        {
                            NetworkStream stream = client.GetStream();
                            if (willClientEnd[Inverse(clientNumbers[client])]){
                                jsonData += "<WEND>WantToEnd</WEND>";
                            }
                            if (willClientEnd[0] && willClientEnd[1]){
                                jsonData += "<RES>Restart</RES>";
                                isRestart = true;
                            }
