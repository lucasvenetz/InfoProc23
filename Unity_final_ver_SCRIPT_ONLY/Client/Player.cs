...
public class Player : MonoBehaviour
{
    ...
    // Player local action related variables
    ...
    // Synchronising related variables
    public TcpClient1 tcpClient;
    public UdpClient1 udpClient1;
    private Vector3[] positionCashe = new Vector3[4];
    private float timeInterval;
    private float timeStart;
    private float timeFinish;
    private float timeElapsed;
    public GameObject[] playersGameObj = new GameObject[2];
    public int[] Keys = new int[7];
    public int[] KeysCashe = new int[7];


    // Action queue related variables
    ...
    void Update()
    {
        if (tcpClient.isStart){
            if (udpClient1.isUpdatePosition){
                ...
                Debug.Log("*** TimeInterval: " + timeInterval + "Start: " + positionCashe[0] + "Final: " + tcpClient.PlayerXTransformPosition[0]);
        ...
        if (tcpClient.isControlled){
            for (int i = 0; i < 7; i++){
                Keys[i] = tcpClient.KeyS[i];
            }
    ...
    void SetMoveSmooth(int playerX){
        positionCashe[playerX] = playersGameObj[playerX].transform.position;
        positionCashe[playerX+2] = udpClient1.PlayerXTransformPosition[playerX];
    }

    void MoveSmooth(int playerX){
        timeElapsed += Time.deltaTime;
        float t = Mathf.Clamp01(timeElapsed / timeInterval / SmoothIndex);
        playersGameObj[playerX].transform.position = Vector3.Lerp(positionCashe[playerX], positionCashe[playerX+2], t);
    }
