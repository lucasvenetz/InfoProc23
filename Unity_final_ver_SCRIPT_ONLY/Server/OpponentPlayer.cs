...
public class OpponentPlayer : MonoBehaviour
{
    ...
    /*
        ...
        if (Input.GetKey(KeyCode.LeftArrow) && moving_state == 0)
        {
            MoveLeft();
            LimitVelocity(walkSpeed);
        }

        if (Input.GetKey(KeyCode.RightArrow) && moving_state == 0)
        {
            MoveRight();
            LimitVelocity(walkSpeed);
        }
        ...
        */
        ...
        for (int i = 0; i < 7; i++){
            Keys[i] = tcpServer1.Keys[1,i]; // ensuring that there is no subroutine synchronising problem and we make a copy
        }

        if (!health.isFailed && tcpServer1.isStart){
            if (KeyChange(1, 6)) 
            {
                Attack();
            }
